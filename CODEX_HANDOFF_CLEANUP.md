...
>
> **브랜치 상태** — `claude/design-warm` tip = **`6c01800`** (이 문서 자체를 얹은 커밋).
> 그 아래가 `5ce0f7b`(리뷰 수정 3건)이고, 코드 변경의 실질적 기준선은 `5ce0f7b`다.
> 즉 이 문서가 설명하는 **코드 상태 = `5ce0f7b`**, **브랜치 tip = `6c01800`(= `5ce0f7b` + 이 문서)**.
> 링크로 바로 들어온 경우 tip과 본문의 커밋 해시가 달라 보일 수 있어 명시한다.
>
> **개정 이력**
> - `6c01800` 최초 작성
> - Codex 1차 리뷰 반영 — §9.4를 순서가 강제되는 단계로 재작성(P1),
>   브랜치 상태 명시(P2), §9.5-1 이미지 훅을 스칼라/복수로 분리(P3).
>   Codex가 §9.1(도달 불가능 판정)은 독립적으로 확인해 줬다.
     ...
     └─ #2 9b6559f  claude/spec-v2-trajectory            v5 궤적 + 통계
         └─ #3 5947de6  claude/design-refresh            디자인 마감
             └─ #4 05135e1  claude/design-editorial      디자인 재설계 + 야간 분리
                 └─ #5 6c01800  claude/design-warm       웜톤 + 팔레트 6종 + 설정 + 리뷰 수정
                        ├ 5ce0f7b  리뷰 수정 3건 (코드 기준선)
                        └ 6c01800  이 문서 (코드 변경 없음)
```
...
**B안이 더 정직할 수 있다.** "정리"와 "기능 복원"은 다른 일이고, 섞으면 둘 다 흐려진다.
A안을 고르면 위 표의 순서를 반드시 지킨다는 조건이 붙는다.
...
1. **IDB 이미지 로딩 이펙트가 4벌 — 단, 네 곳이 같은 모양이 아니다.**
   ...
   **설계**: 복수형을 원시 함수로 두고 스칼라를 파생시킨다.
   ...
   ```js
   useImageUrls(ids)  // → [{id, url}] . 내부에서 ids.join(",")로 이펙트를 건다
   useImageUrl(id)    // → url | "" .   useImageUrls를 감싼 얇은 래퍼
   ```

   **주의 — 이게 P3의 핵심**: 스칼라 래퍼가 매 렌더 `[id]`를 새로 만들면 배열 identity가
   계속 바뀌어 이펙트가 무한 반복한다. 그래서 이펙트 dep은 **배열이 아니라
   `ids.join(",")` 문자열**이어야 한다. 이렇게 하면 지금 `[ids]`·`[photo]`에 있는
   잠재 위험 두 개도 같이 없어진다 — 중복 제거가 아니라 버그 예방이 실제 이득이다.

   `NoteImages`가 §9.4 B안으로 삭제되면 복수형 호출자는 0이 된다. 그때는
   `useImageUrl(id)` 스칼라 하나만 만들고 복수형은 만들지 않는다.
   **즉 이 항목의 최종 형태는 §9.4 결정에 달려 있다.**
...
1. ~~**9.1 판정에 동의하는가?**~~ **✅ 1차 리뷰에서 확인됨** — `origin/claude/design-warm`
   기준으로 `App.jsx:452–473`에서 `!formOnly` 가지가 도달 불가능함을 Codex가 독립 확인했다.

2. **9.4는 A안(2단계 보존 후 삭제)과 B안(같이 삭제 + 별도 기능 PR) 중 어느 쪽인가?**
   1차 리뷰 뒤 이 절을 다시 썼다 — A안을 고르면 **A-1 → A-2 순서가 강제**되고,
   A-1의 E2E가 통과해야만 삭제로 넘어간다. 나는 지금 **B안 쪽으로 기운다**:
   "정리"와 "기능 복원"을 섞지 않는 게 둘 다에 낫다고 본다. 동의하는가?
