...
>
> **코드 기준선 = `5ce0f7b`** (`claude/design-warm`). 이 문서가 서술하는 코드 상태는
> 전부 이 커밋 기준이다. 브랜치 tip은 이 문서가 갱신될 때마다 앞으로 나가므로 여기 적지
> 않는다 — 실제 tip은 `git rev-parse origin/claude/design-warm`으로 확인할 것.
> (초판이 tip을 박아 뒀다가 바로 다음 문서 커밋에 의해 낡았다. 그래서 안 적는다.)
>
> **개정 이력** (해시 없이, 순서만)
> 1. 최초 작성
...
```
1) PR #1 → #2 → #3 → #4 → #5 순서대로 머지          (결정 5)
2) main에서 새 브랜치
3) 커밋 A: 죽은 코드 삭제 (§9.1–9.4, lightbox 포함)  (결정 2) — 순수 삭제
4) 커밋 B: solutionImages 수명주기 수정              (결정 4) — 동작 변경
5) 커밋 C: 셔플 수정                                 (결정 3) — 동작 변경
6) 후속 PR: storage.js:72 방어                       (결정 6)
```
...
필드를 지우지 않고 **세 곳의 누락을 고친다.** 지금은 `migrate.js:99`가 보존만 하고
...
`gcImages`가 셋 중 가장 위험하다. 나머지 둘은 누수(회수 실패)지만 이건 **데이터 손실**이다.
...
| 항목 | 내용 |
|---|---|
| **`storage.js:72` ⚠️ 1순위** | `setItem`이 try/catch 없이 돈다. 바로 위 39줄의 백업 쓰기에는 있다. `useState(loadAll)` 안에서 던지므로 **용량 초과 시 영구 백지 화면** (에러 바운더리 없음). 확률은 낮지만 복구 불가 |
